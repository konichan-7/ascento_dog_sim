...
当前框架只控制世界坐标系竖直力以及机体 roll/pitch，不控制水平速度、轮速或 yaw。模型带有机体 IMU 传感器（`imu_quat` framequat），供后续姿态估计使用，当前 demo 不显示姿态。控制器假设四个车轮均与地面接触；真实机器人后续还需要接触检测、状态估计、驱动器模型和安全状态机。
...
| 监测输出 | 无（`--teleop` 时可通过 Viewer 实时观察运动） |
| 更新频率 | 当前为 1 kHz |
