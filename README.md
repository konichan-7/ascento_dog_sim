...
- 键盘 WASD 遥杆驱动（`vmc --teleop`）：轮速 PI 闭环，前后平移与原地转向。
...
`leg-kinematics` 显示零重力、无接触的单腿悬空运动学；`vmc` 运行启用重力和四轮地面接触的整车闭环控制，并在演示结束后输出 yaw、pitch、roll 曲线。数值验证统一由 `pytest` 负责，不再维护与测试重复的独立验证脚本。加 `--teleop` 可用 W/A/S/D 键盘驱动整车前后平移与原地转向（高度/姿态仍由 VMC 稳定），详见 `docs/drive.md`。
