...
- 键盘遥杆驱动（`vmc --teleop`，数字键 1/2/3/4）：轮速 PI 闭环，前后平移与原地转向，IMU 姿态叠加显示在 viewer 画面内。
...
`leg-kinematics` 显示零重力、无接触的单腿悬空运动学；`vmc` 运行启用重力和四轮地面接触的整车闭环控制，仿真过程中 IMU 测得的 yaw、pitch、roll 叠加显示在 viewer 画面内。数值验证统一由 `pytest` 负责，不再维护与测试重复的独立验证脚本。加 `--teleop` 可用键盘数字键 1/2/3/4 驱动整车前后平移与原地转向（高度/姿态仍由 VMC 稳定，ESC 退出），详见 `docs/drive.md`。
