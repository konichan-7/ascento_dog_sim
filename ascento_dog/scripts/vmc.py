...
def main() -> None:
    ...
    if args.headless:
        while data.time - simulation_start < args.duration:
            _step_and_record(model, data, controller, args, simulation_start, history)
    else:
        _run_with_viewer(model, data, controller, args, simulation_start, history)
...
def _run_with_viewer(model, data, controller, args, simulation_start, history) -> None:
...
def _step_and_record(model, data, controller, args, simulation_start, history) -> None:
    elapsed = float(data.time) - simulation_start
    desired_height = args.height + args.amplitude * sin(
        2.0 * pi * elapsed / args.period
    )
